...
| **Report Compare** (`/api/threads/{id}/report-compare`) | `POST /jobs` - compare Midscene success/failure split reports from thread virtual paths or uploaded zip archives; `GET /jobs/{job_id}` - read the saved comparison result and generated artifact paths |
